...
void insert(int key, int val) {
  ...
  if (!e) panic("[Athul R - ar6316]No memory to allocate bucket!");
...
void * get_phase(void *arg) {
  ...
  printf("[Athul R - ar6316][thread %ld] %ld keys lost!\n", tid, lost);
...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    panic("[Athul R - ar6316]usage: ./parallel_hashtable <num_threads>");
  }
  if ((num_threads = atoi(argv[1])) <= 0) {
    panic("[Athul R - ar6316]must enter a valid number of threads to run");
  }
  ...
  if (!threads) {
    panic("[Athul R - ar6316]out of memory allocating thread handles");
  }
  ...
  printf("[Athul R - ar6316][main] Inserted %d keys in %f seconds\n", NUM_KEYS, end - start);
  ...
  printf("[Athul R - ar6316][main] Retrieved %ld/%d keys in %f seconds\n", NUM_KEYS - total_lost, NUM_KEYS, end - start);
